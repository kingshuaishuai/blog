...
对于上面这段话中进行标记的从因此开始的部分，再说说自己理解，这里告诉我们注解在java的内部帮助我们存储了一些额外数据信息，这些数据可以通过一些工具（注解解释器）进行处理后生成一些描述符文件，也就是通过简单的`@annotation`标记，通过内部处理后就可以帮我们生成一些正常的java代码或者其他数据文件，这些简单的标记可以使我们在java语言的时候更加简洁方便。

> 每当创建涉及重复工作的类或接口时，你通常可以使用注解来自动化和简化流程。例如在 Enterprise JavaBean（EJB）中的许多额外工作就是通过注解来消除的。

其实到目前为止，我对于java中Bean的概念还是很模糊，但通过上面这段描述，可以猜到java中Bean的定义应该可以跟XML和注解扯上点关系，对于Bean方面在后面会单独写文章来描述，毕竟还是不太懂它。
...
**1. 元注解**
在正式说明注解的定义方式之前，我必须先说明另外一个东西，那就是`元注解(meta annotation)`。在上文中我们了解过元数据，是用来描述数据的一种数据，那么元注解也类似，是用来注解其他注解的注解，这样说还是太官方了，其实就是java语言本身已经为我们定义好的一些最基本的注解，我们可以依靠它们来定义我们自己的注解，就像盖房子需要砖，这个元注解就是java为我们准备的转。
